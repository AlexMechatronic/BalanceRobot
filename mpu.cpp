...
int mympu_open(unsigned int rate) {
	...
	ret = dmp_enable_feature(DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_SEND_CAL_GYRO|DMP_FEATURE_GYRO_CAL);
//	ret = dmp_enable_feature(DMP_FEATURE_SEND_CAL_GYRO|DMP_FEATURE_GYRO_CAL);
#ifdef MPU_DEBUG
	if (ret) return 110+ret;
#endif
dmp_enable_gyro_cal(1);//calibra no se debe mover durante 8 segundos
...
int mympu_update() {
	...
	do {
		ret = dmp_read_fifo(gyro,acc,q._l,NULL,&sensors,&fifoCount);
		/* will return:
			0 - if ok
	...
	quaternionToEuler( &q._f, &mympu.ypr.Angles.roll, &mympu.ypr.Angles.pitch, &mympu.ypr.Angles.yaw );
	...
	mympu.ypr.Angles.pitch = -rad2deg(mympu.ypr.Angles.pitch);
	mympu.ypr.Angles.roll = rad2deg(mympu.ypr.Angles.roll);
	mympu.ypr.Angles.yaw = rad2deg(mympu.ypr.Angles.yaw);

	/* need to adjust signs depending on the MPU mount orientation */ 
	mympu.gyro.Angles.pitch = (float)gyro[1] / GYRO_SENS;
	mympu.gyro.Angles.roll = (float)gyro[0] / GYRO_SENS;
	mympu.gyro.Angles.yaw = (float)gyro[2] / GYRO_SENS;
  
  mympu.accel.Vector.x = (float)acc[0] / 16384.0;
  mympu.accel.Vector.y = -(float)acc[1] / 16384.0;
  mympu.accel.Vector.z = (float)acc[2] / 16384.0;
	return 0;
}
